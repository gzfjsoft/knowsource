...
type (
	...
	ServerOrgs struct {
		...
		SrvDiscountId uint64    `db:"srv_discount_id"` // 优惠ID
	}
)
...
func (m *defaultServerOrgsModel) Insert(ctx context.Context, data *ServerOrgs) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.ServerId, data.OrgId, data.SrvDiscountId)
	return ret, err
}
...
func (m *defaultServerOrgsModel) Update(ctx context.Context, data *ServerOrgs) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.ServerId, data.OrgId, data.SrvDiscountId, data.Id)
	return err
}
